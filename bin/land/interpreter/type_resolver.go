...
func (r *TypeResolver) SetVariable(names []string, setValue *ast.Object) error {
	if len(names) == 1 {
		...
		return errors.Errorf("%s is not found in this scope1", names[0])
	} else {
		name := names[0]
		if val, ok := r.Context.Env.Get(name); ok {
			for _, f := range names[1 : len(names)-1] {
				...
				if !ok {
					return errors.Errorf("%s is not found in this scop2", f)
				}
			}
			last := names[len(names)-1]
			_, ok = val.InstanceFields.Get(last)
			if !ok {
				return errors.Errorf("%s is not found in this scope3", last)
			}
		...
		if val, ok := r.Context.Env.Get("this"); ok {
			for _, f := range names[0 : len(names)-1] {
				val, ok = val.InstanceFields.Get(f)
				if !ok {
					return errors.Errorf("%s is not found in this scope4", f)
				}
			}
			last := names[len(names)-1]
			_, ok = val.InstanceFields.Get(last)
			if !ok {
				return errors.Errorf("%s is not found in this scope5", last)
			}
			return setVariable(val, last, setValue)
		}
	...
	if len(names) == 1 {
		...
		if val, ok := r.Context.Env.Get(name); ok {
			for _, f := range names[1:] {
				...
				val, ok = val.InstanceFields.Get(f)
				if !ok {
					return nil, errors.Errorf("%s is not found in this scope9", f)
				}
			}
			return val, nil
		}
		...
		if val, ok := r.Context.Env.Get("this"); ok {
			for _, f := range names[0:] {
				val, ok = val.InstanceFields.Get(f)
				if !ok {
					return nil, errors.Errorf("%s is not found in this scope10", f)
				}
				if val == nil {
					return nil, builtin.NewNullPointerException(f)
				}
		...
		if v, ok := r.Context.StaticField.Get("_", name); ok {
			if val, ok := v.Get(names[1]); ok {
				for _, f := range names[2:] {
					val, ok = val.InstanceFields.Get(f)
					if !ok {
						return nil, errors.Errorf("%s is not found in this scope11", f)
					}
					if val == nil {
						return nil, builtin.NewNullPointerException(f)
					}
		...
		if objMap, ok := r.Context.StaticField.Get(name, names[1]); ok {
			if val, ok := objMap.Get(names[2]); ok {
				for _, f := range names[3:] {
					val, ok = val.InstanceFields.Get(f)
					if !ok {
						return nil, errors.Errorf("%s is not found in this scope12", f)
					}
					if val == nil {
						return nil, builtin.NewNullPointerException(f)
					}
...
func (r *TypeResolver) ResolveMethod(names []string, parameters []*ast.Object) (interface{}, *ast.Method, error) {
	if len(names) == 1 {
		...
		if v, ok := r.Context.Env.Get("this"); ok {
			...
			if method == nil {
				return nil, nil, errors.Errorf("%s is not found in this scope13", methodName)
			}
			return v, method, nil
		}
	} else {
		...
		if val, ok := r.Context.Env.Get(first); ok {
			for _, f := range fields {
				val, ok = val.InstanceFields.Get(f)
				if !ok {
					return nil, nil, errors.Errorf("%s is not found in this scope14", f)
				}
			}
			...
			if v, ok := r.Context.StaticField.Get("_", first); ok {
				if val, ok := v.Get(names[1]); ok {
					for _, f := range names[2 : len(names)-1] {
						val, ok = val.InstanceFields.Get(f)
						if !ok {
							return nil, nil, errors.Errorf("%s is not found in this scope15", f)
						}
					}
					return FindInstanceMethod(val, methodName, parameters, compiler.MODIFIER_ALL_OK)
				}
	...
	return nil, nil, errors.Errorf("%s is not found in this scope16", strings.Join(names, "."))
}
