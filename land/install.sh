...
if [ "$UNAME" != "Linux" -a "$UNAME" != "Darwin" ] ; then
    echo "Sorry, OS not supported: ${UNAME}. Download binary from"
    exit 1
fi
...
if [ "${UNAME}" = "Darwin" ] ; then
  ...
  if [ "${OSX_ARCH}" = "x86_64" ] ; then
    ...
    echo "Sorry, architecture not supported: ${OSX_ARCH}. Download binary from"
    exit 1
  fi
  ...
  if [ "${LINUX_ARCH}" = "i686" ] ; then
    ...
    echo "Sorry, architecture not supported: ${LINUX_ARCH}. Download binary from"
    exit 1
  fi
fi
